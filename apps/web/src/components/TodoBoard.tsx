...
/**
 * The todo board, for every project or for one.
 *
 * There used to be two: this, and a read-only list on the project page that
 * could show todos but never create one — so a project with no todos was a
 * dead end with a link out. One board, two mounts.
 */
export function TodoBoard({ project }: { project?: string } = {}) {
  ...
  // Mounted inside a project, the board is that project's board: the filter
  // is not a chip to dismiss, and "By Project" would group one group.
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-center gap-4 mb-4 flex-wrap">
        ...
        />
        {!scoped && <h1 className="text-xl font-bold">Todos</h1>}
      ...
      {loading ? (
        ...
        // Still the board, not a dead end. The columns and the Add Todo
        // button above stay reachable, because a project with no todos yet
        // is exactly when you want to write the first one.
        <div className="border border-[var(--color-border)] rounded-lg p-6 text-center">
          <p className="text-[var(--color-muted)] text-base mb-1">
            No todos {projectFilter ? 'for this project ' : ''}yet.
          </p>
          <p className="text-[var(--color-muted)] text-sm">
            They are extracted from agent sessions during ingestion — or press
            <button
              onClick={() => setShowCreateForm(true)}
              className="mx-1 px-2 py-0.5 rounded border border-[var(--color-accent)] text-[var(--color-accent)] hover:bg-[var(--color-accent)]/10"
            >+ Add Todo</button>
            to write one.
          </p>
        </div>

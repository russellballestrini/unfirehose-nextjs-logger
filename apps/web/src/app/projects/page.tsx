 ...
 *
 * A two-hundred-line callback inside the page's map, which is how it came to
 * hold thirty-eight branches — git status, expansion, per-project actions,
 * the agent launcher — none of them reachable without driving the page.
 */
export function ProjectRow(props: any) {
  const {
    project, gitStatuses, getAction, expanded, setExpanded, details,
    toggleExpand, commitOne, pushOne, suggestOne, updateAction,
    pendingFileAction, requestFileAction,
  } = props;
    ...
    <div className="space-y-4">
      ...
      <div className="border border-[var(--color-border)] rounded overflow-hidden divide-y divide-[var(--color-border)]">
        {projects.map((project: any) => (
          <ProjectRow
            key={project.name}
            project={project} gitStatuses={gitStatuses} getAction={getAction}
            expanded={expanded} setExpanded={setExpanded} details={details}
            toggleExpand={toggleExpand} commitOne={commitOne} pushOne={pushOne}
            suggestOne={suggestOne} updateAction={updateAction}
            pendingFileAction={pendingFileAction} requestFileAction={requestFileAction}
          />
        ))}
      </div>
    </div>

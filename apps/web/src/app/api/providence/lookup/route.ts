...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const row = db.prepare(`
      SELECT id, answer_text, model, source_type, git_commit, created_at, hit_count
      FROM providence_cache
      WHERE cache_key = ?
    `).get(cache_key) as { id: number, answer_text: string, model: string, source_type: string, git_commit: string | null, created_at: number, hit_count: number } | undefined;

    if (!row) {
      return NextResponse.json({ hit: false, cache_key });
    }

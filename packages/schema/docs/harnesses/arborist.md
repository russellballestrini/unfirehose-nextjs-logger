# Arborist — Harness Format

> Older sessions carry the harness slug `aborist` (a misspelling that shipped); ingest treats it as an alias of `arborist`.
...
Arborist is a coding agent harness that writes unfirehose/1.0 JSONL natively. It was the first harness to ship a production native implementation of our schema, validating that adapters are optional. Sessions land directly in canonical format and flow through ingestion without any transformation step.
...
```
~/.arborist/unfirehose/{project-slug}/{session-uuid}.jsonl
```
...
Arborist writes unfirehose/1.0 JSONL directly. Every object carries `$schema: "unfirehose/1.0"`:

```jsonl
{"$schema":"unfirehose/1.0","type":"session","id":"019506a8-...","projectId":"-home-fox-git-myproject","status":"active","createdAt":"2026-04-12T10:42:45Z","harness":"arborist","harnessVersion":"0.4.2"}
...
Arborist is picked up by `discoverNativeHarnesses()` in `packages/core/db/ingest.ts`. Our discovery rule:
...
By implementing unfirehose/1.0 natively, Arborist:
...
The dashboard's `/api/live` endpoint streams Arborist sessions alongside Claude Code and other native harnesses. Display tags (`'arborist'`, `'claude-code'`, `'agnt'`) come from our directory name, not a hardcoded list. Ingestion result statistics (projects added, sessions added, messages added, blocks added) flow into our usual ingest pipeline.

## Implementation Checklist

For other harnesses adopting unfirehose/1.0 natively, follow Arborist's pattern:
